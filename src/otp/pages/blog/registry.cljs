(ns otp.pages.blog.registry
  "Post registry — maps slug strings to post metadata and render components.
   To add a new post:
   1. Create a namespace under otp.pages.blog.posts.*
   2. Export a `post-meta` map and a `post-content` defnc component.
   3. Require the namespace here and add an entry to `posts`."
  (:require
   [otp.pages.blog.posts.pavilion-construction-begins :as post-1]
   [otp.pages.blog.posts.blue-yellow-maquette :as post-byby]
   [otp.pages.blog.posts.venice-arrival :as post-2]))
